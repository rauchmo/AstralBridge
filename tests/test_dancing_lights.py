...
import dancing_lights.layers as dl_layers
import dancing_lights.devices as dl_devices
import dancing_lights.config as dl_config
...
def reset_globals():
    ...
    dl_layers._dl_ds_ambient_mode = None
    dl_layers._dl_ds_player_active = None
    timer = dl_layers._dl_ds_roll_timer
...
def mock_wled(monkeypatch):
    ...
    monkeypatch.setattr(dl_devices, "_dl_set", fake_set)
    monkeypatch.setattr(dl_devices, "_dl_get", fake_get)
    monkeypatch.setattr("dancing_lights.layers._dl_set", fake_set)
    monkeypatch.setattr("dancing_lights.routes._dl_set", fake_set)
    return calls
...
def ds_config(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dl_config, "DL_CONFIG_FILE", config_file)
    return cfg
...
def test_dl_load_adds_ambient_defaults_when_missing(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(dl_config, "DL_CONFIG_FILE", f)
...
async def test_apply_layer_signal_overrides_ambient(mock_wled, ds_config):
    dl_layers._dl_ds_ambient_mode = "tavern"
    dl_layers._dl_ds_player_active = "p1"
    await dl.dl_ds_apply_current_layer()
    ip, state = mock_wled[0]
...
async def test_apply_layer_skips_write_during_roll(mock_wled, ds_config):
    dl_layers._dl_ds_ambient_mode = "tavern"
    dl_layers._dl_ds_roll_timer = asyncio.create_task(asyncio.sleep(100))
    await dl.dl_ds_apply_current_layer()
...
async def test_ds_signal_sets_player_active_and_applies(mock_wled, ds_config):
    ...
    assert dl_layers._dl_ds_player_active == "p1"
    assert len(mock_wled) == 1
    ip, state = mock_wled[0]
    seg_ids = [s.get("id", 0) for s in state["seg"]]
    assert 1 in seg_ids
...
async def test_ds_signal_unknown_player_noop(mock_wled, ds_config):
    ...
    assert dl_layers._dl_ds_player_active is None
...
async def test_ds_clear_restores_ambient(mock_wled, ds_config):
    dl_layers._dl_ds_player_active = "p1"
    dl_layers._dl_ds_ambient_mode = "dungeon"
...
async def test_ambient_set_activates_mode_and_applies(mock_wled, ds_config):
    ...
    assert dl_layers._dl_ds_ambient_mode == "forest"
    ip, state = mock_wled[0]
...
async def test_ambient_clear_removes_mode(mock_wled, ds_config):
    ...
    assert dl_layers._dl_ds_ambient_mode is None
    cfg = dl.dl_load()
...
async def test_trigger_sets_roll_timer(mock_wled, ds_config):
    await dl.dl_trigger("nat20")
    assert dl_layers._dl_ds_roll_timer is not None
    dl_layers._dl_ds_roll_timer.cancel()
    dl_layers._dl_ds_roll_timer = None
...
async def test_trigger_cancels_previous_timer_on_rapid_fire(mock_wled, ds_config):
    await dl.dl_trigger("nat20")
    first_timer = dl_layers._dl_ds_roll_timer
...
def test_delete_ambient_clears(api_client):
    dl_layers._dl_ds_ambient_mode = "combat"
...
def test_get_dungeon_screen_includes_ambient_state(api_client):
    dl_layers._dl_ds_ambient_mode = "magic"
...
def test_restore_endpoint_applies_current_layer(api_client, mock_wled):
    dl_layers._dl_ds_ambient_mode = "ice"
...
def test_manual_mode_default_is_false():
    ...
    assert dl_layers._dl_manual_mode is False
...
def test_post_mode_sets_manual_true(api_client, ds_config):
    ...
    assert dl_layers._dl_manual_mode is True
...
def ha_http(monkeypatch, ds_config):
    ...
    monkeypatch.setattr(dl_devices.httpx, "AsyncClient", FakeClient)
    return posts
...
def mock_ha_set(monkeypatch):
    ...
    async def fake(entity_id, state, ha_effect=""):
        ...
        return True
    monkeypatch.setattr(dl_devices, "_ha_set", fake)
    return calls
...
def ha_dev_config(dev_config, mock_ha_set):
    ...
    dl.dl_save(cfg)
    dl_layers._dev_ambient["ha001"] = None
    dl_layers._dev_manual["ha001"] = False
    return dl.dl_load()


@pytest.mark.asyncio
async def test_dev_apply_ambient_ha_uses_ha_set(mock_ha_set, ha_dev_config):
    dl_layers._dev_ambient["ha001"] = "tavern"
    await dl_layers._dev_apply_ambient("ha001")
    assert len(mock_ha_set) == 1
    assert mock_ha_set[0]["entity_id"] == "light.table"
    assert mock_ha_set[0]["ha_effect"] == "colorloop"
...
async def test_dev_trigger_ha_uses_ha_set(mock_ha_set, ha_dev_config):
    ...
    ev = dev["events"]["nat20"]
    await dl_layers._dev_trigger("ha001", dev, ev)
    assert len(mock_ha_set) == 1
    assert mock_ha_set[0]["entity_id"] == "light.table"
    assert mock_ha_set[0]["ha_effect"] == "flash"
    t = dl_layers._dev_roll_timers.pop("ha001", None)
    if t: t.cancel()
